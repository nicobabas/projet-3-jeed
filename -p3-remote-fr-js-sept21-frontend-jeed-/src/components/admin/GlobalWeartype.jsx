...
import CreateWeartype from './CreateWeartype';

const GlobalWeartype = () => {
  ...
  const deleteWeartype = (id) => {
    axios.delete(`http://localhost:8000/weartypes/${id}`).then(({ data }) => {
        ...
        <table className="table-fixed min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {weartypes?.map((weartype) => (
              <tr className="hover:bg-gray-100" key={weartype.id}>
                <td className="p-4 w-4">{weartype.id}</td>
                <td className="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                  {weartype.name}
                </td>
                <td className="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                  <img src={weartype.image} alt={weartype.name} width="50" />
                </td>
                <td className="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                  <button
                    onClick={() => deleteWeartype(weartype.id)}
                    type="button"
