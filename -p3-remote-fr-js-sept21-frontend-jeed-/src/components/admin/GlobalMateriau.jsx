...
const GlobalMateriau = () => {
  ...
  const getData = () => {
    axios.get('http://localhost:8000/materiau').then((response) => {
      setMateriau(response.data);
    });
  };
  const deleteMateriau = (id) => {
    axios.delete(`http://localhost:8000/materiau/${id}`).then(({ data }) => {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {materiau?.map((item) => (
              <tr className="hover:bg-gray-100" key={item.id}>
                ...
                <td className="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                  <button
                    onClick={() => deleteMateriau(item.id)}
                    type="button"
