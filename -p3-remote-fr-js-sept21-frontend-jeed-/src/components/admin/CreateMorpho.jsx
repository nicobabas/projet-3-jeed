...
const url = 'http://localhost:8000/morphos/';
...
const CreateMorpho = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (name) {
      const newMorpho = { name: name };
      axios.post(url, newMorpho).then(({ data }) => {
                ...
                <div className="px-4 py-5 bg-white space-y-6 sm:p-6">
                  <div className="col-span-6 sm:col-span-6 lg:col-span-2">
                    <h2 className="mb-5 text-lg font-medium leading-6 text-gray-900">
                      Créer ou supprimer un style morphologique
                    </h2>
                    <label
                      htmlFor="name"
