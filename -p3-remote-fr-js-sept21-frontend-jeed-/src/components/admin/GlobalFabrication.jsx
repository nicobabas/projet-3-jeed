...
import CreateFabrication from './CreateFabrication';

const GlobalFabrication = () => {
  ...
  const getData = () => {
    axios.get('http://localhost:8000/fabrications').then((response) => {
      setFabrication(response.data);
    });
  };
  const deleteFabrication = (id) => {
    axios
      .delete(`http://localhost:8000/fabrications/${id}`)
      .then(({ data }) => {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {fabrication?.map((item) => (
              <tr className="hover:bg-gray-100" key={item.id}>
                ...
                <td className="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                  <button
                    onClick={() => deleteFabrication(item.id)}
                    type="button"
...
export default GlobalFabrication;
